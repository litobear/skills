...
def format_results(results, currency, is_round_trip=False):
    ...
    for i, result in enumerate(results, 1):
        if is_round_trip and isinstance(result, tuple):
            # Round-trip results from search_with_currency are:
            #   ((outbound_flight, return_flight), booking_token)
            # Keep backward compatibility with plain (outbound, return).
            if len(result) == 2 and isinstance(result[0], tuple):
                outbound, ret = result[0]
            else:
                outbound, ret = result
            print(f"\n{'='*60}")
